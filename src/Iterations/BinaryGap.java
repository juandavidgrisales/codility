...
/**
 * Created by juandavidgrisales on 31/05/17.
 */
public class BinaryGap {

    public BinaryGap () {
        int number = 2147483647;
        System.out.println("the longest 'Binary Gap' for " + number + " is " + solution(number));
    }

    public int solution(int N) {
        int binaryGap = 0;

        String nBinaryWithZeros = Integer.toBinaryString(N);
        int limitOfBinaryGap = nBinaryWithZeros.lastIndexOf('1');
        String nBinary = nBinaryWithZeros.substring(0, limitOfBinaryGap+1);
        String[] binaryGaps = nBinary.split("1");

        for (String binaryGapString : binaryGaps) {
            if (binaryGap < binaryGapString.length())
                binaryGap = binaryGapString.length();
        }
